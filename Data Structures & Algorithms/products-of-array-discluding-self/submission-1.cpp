...
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        ...
        // revised to change ans while computing fwd and bwd products
        for (int idx{0}; idx < n; ++idx){
            fwd_prod[idx+1] = nums[idx] * fwd_prod[idx];
            ans[idx] *= fwd_prod[idx];
        }
        for (int idx{n-1}; idx >= 0; --idx){
            bwd_prod[idx] = nums[idx] * bwd_prod[idx+1]; 
            ans[idx] *= bwd_prod[idx+1];
        }
